...
from schemas.user import User_Record, User_List, User_Activate, User_Read, User_ResetPassword
...
async def create_user(user: User_Record, db: Session = Depends(get_db)): #db: Session = Depends(SessionLocal)):
...
async def leer_usuarios(usuario_actual: Annotated[User_Record, Depends(get_current_active_user)],
...
async def eliminar_usuario(usuario_actual: Annotated[User_Record, Security(get_current_user, scopes=["admin"])],
...
async def activar_usuario(usuario_actual: Annotated[User_Record, Security(get_current_user, scopes=["admin"])],
...
async def actualizar_usuario(usuario_actual: Annotated[User_Record, Security(get_current_user, scopes=["admin"])], 
...
async def actualizar_contrasenna(usuario_actual: Annotated[User_Record, Security(get_current_user, scopes=["profesor", "cliente", "estudiante"])],
